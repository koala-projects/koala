...
public class UserApiTest {
  ...
  public void crud() throws Exception {
    ...
    mockMvc.perform(get("%s/%s".formatted(url, entity.getId())))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.data.nickname", equalTo("test2")));
  ...
  @Test
  @WithMockUser(username = "admin", authorities = {"user:read", "user:write"})
  public void departmentIds() throws Exception {
    String url = "/api/users";
    UserEntity entity = UserEntity.builder()
      .id("999")
      .username("test")
    ...
    mockMvc.perform(put("%s/%s/department-ids".formatted(url, entity.getId()))
      .contentType(MediaType.APPLICATION_JSON)
      .content(mapper.writeValueAsString(DataRequest.builder().data(List.of("999")).build()))
    ).andExpect(status().isOk());
    mockMvc.perform(get(("%s/%s/department-ids").formatted(url, entity.getId())))
      .andExpect(status().isOk())
  ...
  @Test
  @WithMockUser(username = "admin", authorities = {"user:read", "user:write"})
  public void roleIds() throws Exception {
    String url = "/api/users";
    UserEntity entity = UserEntity.builder()
      .id("999")
      .username("test")
    ...
    mockMvc.perform(put("%s/%s/role-ids".formatted(url, entity.getId()))
      .contentType(MediaType.APPLICATION_JSON)
      .content(mapper.writeValueAsString(DataRequest.builder().data(List.of("999")).build()))
    ).andExpect(status().isOk());
    mockMvc.perform(get(("%s/%s/role-ids").formatted(url, entity.getId())))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.data", hasSize(1)))
      .andExpect(jsonPath("$.data[0]", equalTo("999")));
  }
