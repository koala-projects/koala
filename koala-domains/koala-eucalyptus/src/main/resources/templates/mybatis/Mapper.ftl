...
<!-- namespace必须指向Dao接口 -->
<mapper namespace="${packageName}.repositories.${domain.className}Repository">
  <sql id="select${domain.className}">
    select
  ...
  <insert id="add" parameterType="${packageName}.entities.${domain.className}Entity">
  ...
  <update id="delete" parameterType="${packageName}.entities.${domain.className}Entity">
  ...
  <select id="findAll" resultType="${packageName}.entities.${domain.className}Entity">
    <include refid="select${domain.className}"/>
    <where>
  ...
  <select id="findById" resultType="${packageName}.entities.${domain.className}Entity">
    <#if domain.idProperty??>
    <include refid="select${domain.className}"/>
    where t.${domain.idProperty.column.name} = #{${domain.idProperty.code}, jdbcType=${domain.idProperty.column.type}}
    </#if>
  </select>
  <update id="update" parameterType="${packageName}.entities.${domain.className}Entity">
    <#if domain.idProperty??>
